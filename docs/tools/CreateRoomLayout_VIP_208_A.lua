-- Studio helper: create the VIP Gallery Suite owned-room layout template.
--
-- Run this manually in Roblox Studio Command Bar or from a temporary Script.
-- The helper creates ReplicatedStorage.RoomTemplates.RoomLayout_VIP_208_A.
--
-- VIP Gallery Suite is a simple rectangular 13x16 VIP room with 208 walkable
...
local TOOL_PREFIX = "[CreateRoomLayout_VIP_208_A]"
...
local LAYOUT_ID = "VIP_208_A"
local TEMPLATE_NAME = "RoomLayout_VIP_208_A"
local DISPLAY_NAME = "VIP Gallery Suite"
...
local COLORS = {
	...
	FloorVein = Color3.fromRGB(190, 184, 174),
	FloorAccent = Color3.fromRGB(239, 234, 221),
	Wall = Color3.fromRGB(234, 226, 214),
	WallPanel = Color3.fromRGB(209, 198, 180),
	GoldTrim = Color3.fromRGB(204, 158, 64),
	DarkTrim = Color3.fromRGB(92, 68, 37),
	Entrance = Color3.fromRGB(93, 80, 67),
	EntranceEdge = Color3.fromRGB(203, 164, 78),
	Platform = Color3.fromRGB(217, 210, 198),
	PlatformEdge = Color3.fromRGB(176, 132, 54),
}
...
local function buildDecorativeFloor(roomFolder)
	...
	local floorStyle = buildFloorStyle(decorativeFloor)

	if floorStyle == "Plain" then
		createVisualPart(
			decorativeFloor,
			"LongMarbleInlay",
			Vector3.new(TILE_SIZE * 5, 0.035, TILE_SIZE * 7),
			CFrame.new(0, FLOOR_TOP_Y + 0.035, TILE_SIZE * 1.5),
			COLORS.FloorAccent,
			Enum.Material.Marble
		)
		...
		createVisualPart(
			decorativeFloor,
			"LongMarbleInlayGoldFront",
			Vector3.new(TILE_SIZE * 5 + 0.3, 0.04, 0.14),
			CFrame.new(0, FLOOR_TOP_Y + 0.06, -TILE_SIZE * 2),
			COLORS.GoldTrim,
			Enum.Material.Metal
		)
		...
		createVisualPart(
			decorativeFloor,
			"LongMarbleInlayGoldBack",
			Vector3.new(TILE_SIZE * 5 + 0.3, 0.04, 0.14),
			CFrame.new(0, FLOOR_TOP_Y + 0.06, TILE_SIZE * 5),
			COLORS.GoldTrim,
			Enum.Material.Metal
		)
		...
		createVisualPart(
			...
			CFrame.new(-TILE_SIZE * 2.75, FLOOR_TOP_Y + 0.04, TILE_SIZE * 0.5),
			COLORS.FloorVein,
			Enum.Material.Marble
		)
...
local function buildDecor(roomFolder)
	...
	for panelIndex = 1, 3 do
		local panelX = (panelIndex - 2) * TILE_SIZE * 2.3

		createVisualPart(
			decor,
			"BackWallPanel_" .. tostring(panelIndex),
			Vector3.new(TILE_SIZE * 1.6, 2.4, 0.12),
	...
	createVisualPart(
		decor,
		"VisualSideBalcony",
		Vector3.new(TILE_SIZE * 2.5, 0.12, TILE_SIZE * 5),
		CFrame.new(halfWidth - TILE_SIZE * 1.25, FLOOR_TOP_Y + 0.08, halfDepth - TILE_SIZE * 2.5),
		COLORS.Platform,
		Enum.Material.Marble
	)
	...
	createVisualPart(
		decor,
		"VisualBalconyGoldRail",
		Vector3.new(0.18, 0.4, TILE_SIZE * 5),
		CFrame.new(halfWidth - TILE_SIZE * 2.5, FLOOR_TOP_Y + 0.34, halfDepth - TILE_SIZE * 2.5),
		COLORS.PlatformEdge,
		Enum.Material.Metal
	)
	...
	createVisualPart(
		decor,
		"VisualBalconyGoldFront",
		Vector3.new(TILE_SIZE * 2.5, 0.16, 0.16),
		CFrame.new(halfWidth - TILE_SIZE * 1.25, FLOOR_TOP_Y + 0.17, halfDepth - TILE_SIZE * 5),
		COLORS.PlatformEdge,
		Enum.Material.Metal
	)
	...
	for stepIndex = 1, 4 do
		createVisualPart(
			decor,
			"VisualStairRiser_" .. tostring(stepIndex),
			Vector3.new(TILE_SIZE * 1.8, 0.08, 0.3),
			CFrame.new(
				-halfWidth + TILE_SIZE * 1.7,
				FLOOR_TOP_Y + 0.06 + stepIndex * 0.035,
				halfDepth - TILE_SIZE * 4.6 + stepIndex * 0.4
			),
			COLORS.GoldTrim,
			Enum.Material.Metal
		)
...
print(TOOL_PREFIX .. " VIP balcony and stair/riser details are visual-only, non-colliding, non-touching, and non-querying.")
