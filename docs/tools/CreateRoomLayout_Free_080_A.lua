-- Studio helper: create the Open Studio owned-room layout template.
--
-- Run this manually in Roblox Studio Command Bar or from a temporary Script.
-- The helper creates ReplicatedStorage.RoomTemplates.RoomLayout_Free_080_A.
--
-- Open Studio is a simple rectangular 10x8 free room with 80 walkable tiles.
-- It intentionally has no stairs, holes, hidden areas, multi-level structures,
-- or default furniture.

local ReplicatedStorage = game:GetService("ReplicatedStorage")

local TOOL_PREFIX = "[CreateRoomLayout_Free_080_A]"
local REPLACE_EXISTING = true

local LAYOUT_ID = "Free_080_A"
local TEMPLATE_NAME = "RoomLayout_Free_080_A"
local DISPLAY_NAME = "Open Studio"
...
local COLORS = {
	Floor = Color3.fromRGB(190, 183, 166),
	FloorGrid = Color3.fromRGB(169, 161, 145),
	Wall = Color3.fromRGB(220, 212, 193),
	WallTrim = Color3.fromRGB(145, 125, 94),
	DarkTrim = Color3.fromRGB(88, 72, 52),
	Entrance = Color3.fromRGB(132, 146, 142),
	EntranceEdge = Color3.fromRGB(83, 102, 99),
}
...
print(string.format(
	"%s LayoutId=%s TileCount=%d Grid=%dx%d TileSize=%d WalkableFloorSize=(%.1f, %.1f, %.1f)",
	TOOL_PREFIX,
	LAYOUT_ID,
	TILE_COUNT,
	GRID_WIDTH,
...
print(string.format("%s DoorTileIndex=%d DoorCenterX=%.1f cave width = %.0f tile (%.1f studs).", TOOL_PREFIX, DOOR_TILE_INDEX, DOOR_CENTER_X, DOOR_OPENING_WIDTH / TILE_SIZE, DOOR_OPENING_WIDTH))
print(string.format("%s EntryWalkTarget remains first interior tile: z=%.1f.", TOOL_PREFIX, entryWalkTarget.Position.Z))
print(TOOL_PREFIX .. " Doorway trim and entrance-edge visuals are non-colliding.")
