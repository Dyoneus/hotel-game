-- Studio helper: create the VIP Marble Studio owned-room layout template.
--
-- Run this manually in Roblox Studio Command Bar or from a temporary Script.
-- The helper creates ReplicatedStorage.RoomTemplates.RoomLayout_VIP_080_A.
--
-- VIP Marble Studio is a simple rectangular 8x10 VIP room with 80 walkable
-- tiles. It keeps the current movement system safe with one flat WalkableFloor.
-- Premium stair, riser, and platform details are visual-only placeholders.

local ReplicatedStorage = game:GetService("ReplicatedStorage")

local TOOL_PREFIX = "[CreateRoomLayout_VIP_080_A]"
...
local LAYOUT_ID = "VIP_080_A"
local TEMPLATE_NAME = "RoomLayout_VIP_080_A"
local DISPLAY_NAME = "VIP Marble Studio"
...
local COLORS = {
	...
	FloorVein = Color3.fromRGB(191, 184, 173),
	FloorAccent = Color3.fromRGB(238, 232, 218),
	Wall = Color3.fromRGB(233, 226, 213),
	WallPanel = Color3.fromRGB(207, 196, 176),
	GoldTrim = Color3.fromRGB(202, 157, 66),
	DarkTrim = Color3.fromRGB(95, 70, 36),
...
local function buildDecorativeFloor(roomFolder)
	...
	if floorStyle == "Plain" then
		createVisualPart(
			decorativeFloor,
			"MarbleCenterInlay",
			Vector3.new(TILE_SIZE * 3, 0.035, TILE_SIZE * 2),
			CFrame.new(0, FLOOR_TOP_Y + 0.035, TILE_SIZE),
			COLORS.FloorAccent,
			Enum.Material.Marble
		)
		...
		createVisualPart(
			decorativeFloor,
			"MarbleCenterInlayGoldEdge",
			Vector3.new(TILE_SIZE * 3 + 0.25, 0.04, 0.12),
...
local function buildDecor(roomFolder)
	...
	local panelY = FLOOR_TOP_Y + WALL_HEIGHT * 0.42
	...
	createVisualPart(
		decor,
		"BackWallPanelLeft",
		Vector3.new(TILE_SIZE * 2.2, 2.2, 0.12),
		CFrame.new(-TILE_SIZE * 2.2, panelY, halfDepth + 0.04),
		COLORS.WallPanel,
		Enum.Material.Marble
	)
	...
	createVisualPart(
		decor,
		"VisualRaisedPlatform",
		Vector3.new(TILE_SIZE * 2.5, 0.12, TILE_SIZE * 2),
		CFrame.new(halfWidth - TILE_SIZE * 1.25, FLOOR_TOP_Y + 0.08, halfDepth - TILE_SIZE),
		COLORS.Platform,
		Enum.Material.Marble
	)
	...
	createVisualPart(
		decor,
		"VisualPlatformGoldFront",
		Vector3.new(TILE_SIZE * 2.5, 0.16, 0.16),
		CFrame.new(halfWidth - TILE_SIZE * 1.25, FLOOR_TOP_Y + 0.17, halfDepth - TILE_SIZE * 2),
		COLORS.PlatformEdge,
		Enum.Material.Metal
	)
	...
	for stepIndex = 1, 3 do
		createVisualPart(
			...
			Vector3.new(TILE_SIZE * 1.4, 0.08, 0.28),
			CFrame.new(
				-halfWidth + TILE_SIZE * 1.4,
				FLOOR_TOP_Y + 0.06 + stepIndex * 0.035,
				halfDepth - TILE_SIZE * 2.2 + stepIndex * 0.35
			),
			COLORS.GoldTrim,
			Enum.Material.Metal
		)
...
print(TOOL_PREFIX .. " VIP platform and stair/riser details are visual-only, non-colliding, non-touching, and non-querying.")
